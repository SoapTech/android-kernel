...
#ifdef CONFIG_HAS_EARLYSUSPEND
static void kxcj9_early_suspend(struct early_suspend *es);
static void kxcj9_early_resume(struct early_suspend *es);
#endif
...
static void kxcj9_early_suspend(struct early_suspend *es)
{
	...
	acceld = container_of(es, struct kionix_accel_driver, early_suspend);
	...
	err = i2c_smbus_write_byte_data(acceld->client,
					ACCEL_GRP4_DATA_CTRL, acceld->accel_registers[accel_grp4_ctrl_reg1] &= ~ACCEL_GRP4_DRDYE);
	if (err < 0)
		return err;
	err = i2c_smbus_write_byte_data(acceld->client,
					ACCEL_GRP2_CTRL_REG1, 1);
	if (err < 0)
		return err;
	#endif
...
{
    pr_info("kionix_accel_power_off\n");
				 ...
				 const struct i2c_device_id *id)
{
    ...
    pr_info("kionix accel probe begin...\n");
	...
	#if 1
	#ifdef CONFIG_HAS_EARLYSUSPEND
		acceld->early_suspend.suspend = kxcj9_early_suspend;
		acceld->early_suspend.resume  = kxcj9_early_resume;
...
{
    pr_info("kionix accel driver init...\n");
