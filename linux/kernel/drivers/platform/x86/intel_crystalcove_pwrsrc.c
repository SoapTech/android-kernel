...
int crystal_cove_unmask_vbus_intr(void)
{
	...
	intel_mid_pmic_clearb(CRYSTALCOVE_MPWRSRCIRQS0_REG, 0x01);
	intel_mid_pmic_clearb(CRYSTALCOVE_MPWRSRCIRQSX_REG, 0x01);

	return 0;
}
EXPORT_SYMBOL(crystal_cove_unmask_vbus_intr);

int crystal_cove_mask_vbus_intr(void)
{
	/* mask the PWRSRC interrupts */
	intel_mid_pmic_setb(CRYSTALCOVE_MPWRSRCIRQS0_REG, 0x01);
	intel_mid_pmic_setb(CRYSTALCOVE_MPWRSRCIRQSX_REG, 0x01);

	return 0;
}
EXPORT_SYMBOL(crystal_cove_mask_vbus_intr);
					...
					unsigned long event, void *data)
{
	...
	if (extcon_num_of_cable_devs(EXTCON_CABLE_SDP)) {
		dev_info(&info->pdev->dev, "unregistering extcon device\n");
...
static int pwrsrc_extcon_registration(struct pwrsrc_info *info)
{
	...
	if (extcon_num_of_cable_devs(EXTCON_CABLE_SDP)) {
		...
		dev_info(&info->pdev->dev, "registering extcon device\n");
