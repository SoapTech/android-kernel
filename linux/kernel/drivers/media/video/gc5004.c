...
//	printk("**jk** read reg: reg = %x, data = %x\n", reg, data[0]);

	return 0;

error:
	dev_err(&client->dev, "**jk**read from offset 0x%x error %d", reg, err);
	return err;
}
...
//		data[1] = (u8)(val);
	...
	if (reg == 0xb0 || reg == 0xb1 || reg == 0xb2  || reg == 0xb6|| reg == 0x3 || reg ==0x4)	
	{
//		printk("**jk666** write reg = %x, val = %x\n", reg, val);
	...
	if (ret)
		printk("**jk666** write error: wrote 0x%x to offset 0x%x error %d",
			val, reg, ret);
				   ...
				   const struct gc5004_reg *reglist)
{
	...
	for (; next->type != GC5004_TOK_TERM; next++) {
		...
		default:
//			printk("**jk** write reg: next->sreg = %x, next->val = %x \n", next->sreg, next->val);
				   ...
				   struct v4l2_private_int_data *priv)
{
	...
	if (IS_ERR(dev->otp_data)) {
		printk("gc5004_g_priv_int_data: no otp data ++\n");
...
fail_power:
	...
	dev_err(&client->dev, "**jk**sensor power-up failed\n");
...
////////////////////////////////////////////////
   ...
   gc5004_write_reg(client, GC5004_8BIT, 0x03, 0x07); //10fps
   ...
   gc5004_write_reg(client, GC5004_8BIT, 0x08, 0x1c); //4µÄ±¶Êý
   ...
   gc5004_write_reg(client, GC5004_8BIT, 0x10, 0x40);  
   gc5004_write_reg(client, GC5004_8BIT, 0x17, 0x14); //01//14//[0]mirror [1]flip
...
////////////////////////////////////////////////
   ...
   gc5004_write_reg(client, GC5004_8BIT, 0x94, 0x00); //04/crop win x
...
//2592x1944
	...
	gc5004_write_reg(client, GC5004_8BIT, 0xfc, 0x06);
	gc5004_write_reg(client, GC5004_8BIT, 0xf7, 0x37); //pll enable
	gc5004_write_reg(client, GC5004_8BIT, 0xf8, 0x93); //Pll mode 2
	gc5004_write_reg(client, GC5004_8BIT, 0xf9, 0xfe); //[0] pll enable  change at 17:37 04/19
	gc5004_write_reg(client, GC5004_8BIT, 0xfa, 0x11); //div
	gc5004_write_reg(client, GC5004_8BIT, 0xfe, 0x00);
...
//		return ret;
	...
	vt_pix_clk_freq_mhz = 91200000;//ext_clk_freq_hz / 2;
			      ...
			      struct v4l2_mbus_framefmt *fmt)
{
	...
	ret = __gc5004_update_exposure_timing(client, dev->coarse_itg,
		dev->pixels_per_line, dev->lines_per_frame);
//	printk("**jk** mbus fmt: gc5004_def_reg = %x, dev->coarse_itg = %x, dev->fps = %x, dev->pixels_per_line = %x, dev->lines_per_frame = %x\n", gc5004_def_reg, dev->coarse_itg, dev->fps,dev->pixels_per_line, dev->lines_per_frame);
	if (ret)
		goto out;
...
//	printk("**jk** gc5004 detect \n");	
	...
	*id = *id | (u16)id_l;
	...
	printk("**jk** 22 gc5004 detect id = %x\n", *id);	
	if (*id == GC5004_ID)
		goto found;
	else {
...
static struct i2c_driver gc5004_driver = {
	...
	.probe = gc5004_probe,
	.remove = __devexit_p(gc5004_remove),
	.id_table = gc5004_ids,
};
