			...
			struct sh_css_binary *binary)
{
	...
	// TODO: buffers_per_line is not used after this. remove? or is this an error?
	//if (!buffers_per_line)
		...
		if ((binary && binary->vf_frame_info.res.width) ||
			...
			if (ret != IA_CSS_SUCCESS) {
				if (stage->out_frame_allocated) {
					...
					/* out_frame = NULL; TODO: should out_frame be a ** arg and set to NULL */
				}
				sh_css_free(stage);
				return ret;
			}
...
find_pipe_by_num(uint8_t pipe_num)
{
	unsigned int i;
	for (i = 0; i < MAX_NUM_PIPES; i++){
		if (my_css.active_pipes[i] && (my_css.active_pipes[i]->pipe_num == pipe_num)) {
...
/* start of test for using rmgr for acq/rel memory */
	...
	assert(h_vbuf != NULL);
	if (h_vbuf == NULL) /* defensive programming; fixes KW issue */
		return IA_CSS_ERR_INTERNAL_ERROR;
	assert(h_vbuf->vptr != 0x0);
	if (h_vbuf->vptr == 0x0) /* defensive programming; fixes KW issue */
		return IA_CSS_ERR_INTERNAL_ERROR;
...
construct_capture_pipe(struct sh_css_pipe *pipe)
{
	...
	if (need_pp) {
		...
		if (post_stage == NULL) /* add_capture_pp_stage assumes non-NULL post_stage */
			return IA_CSS_ERR_INTERNAL_ERROR;
...
ia_css_stream_send_input_frame(const struct ia_css_stream *stream,
			       ...
			       unsigned int height)
{
	assert(stream != NULL); // TODO: should we test this and return an error ?
...
ia_css_stream_start(struct ia_css_stream *stream)
{
	...
	if (stream->last_pipe == NULL)
		return IA_CSS_ERR_INVALID_ARGUMENTS;
	if (stream->last_pipe->old_pipe == NULL)
		return IA_CSS_ERR_INVALID_ARGUMENTS;
