...
//Add by emdoor jim.kuang for pmic read
//2014-04-06
#include <linux/mfd/intel_mid_pmic.h>
struct fsa9285_chip *mcharge_chip=NULL;
...
int fsa_cabletype;
int fsa_cable_type(void)
{
	return fsa_cabletype;
}
...
static int fsa9285_detect_dev(struct fsa9285_chip *chip)
{
	...
	printk("Justin.Zhao: HARDCODE %s, stat=0x%x, ohm_code=0x%x, devtype=0x%x.\r\n", __func__, stat, ohm_code, devtype);
	...
	if (stat & STATUS_ID_SHORT) {
		...
		if (ohm_code == OHM_CODE_UART) {
			...
			fsa_cabletype = POWER_SUPPLY_CHARGER_TYPE_USB_ACA;
		...
		if (!(stat & 0x01)) {
			dev_info(&chip->client->dev, "SE1 cable connected.....\n");
			notify_charger = true;
			cable = FSA9285_EXTCON_DCP;
			fsa_cabletype = POWER_SUPPLY_CHARGER_TYPE_USB_DCP;
			if (!wake_lock_active(&chip->wakelock))
				wake_lock(&chip->wakelock);
		} else {
			dev_info(&chip->client->dev,
			...
			notify_otg = true;
			vbus_mask = 1;
			fsa_cabletype = POWER_SUPPLY_CHARGER_TYPE_USB_SDP;
		}
		...
		fsa_cabletype = POWER_SUPPLY_CHARGER_TYPE_USB_CDP;
		if (!wake_lock_active(&chip->wakelock))
			wake_lock(&chip->wakelock);
		...
		fsa_cabletype = POWER_SUPPLY_CHARGER_TYPE_ACA_DOCK;
		if (!wake_lock_active(&chip->wakelock))
			wake_lock(&chip->wakelock);
	} else {
		#if 0
	...
	if (discon_evt) {
		vbus_mask = 0;	
		fsa_cabletype = POWER_SUPPLY_CHARGER_TYPE_NONE;
		...
		dev_info(&chip->client->dev, "cable connect event\n");
	}
...
static irqreturn_t fsa9285_irq_handler(int irq, void *data)
{
	...
	printk("Justin.Zhao: %s, %d", __func__, irq);
...
//Add by emdoor jim.kuang 2014-04-10
u8 reg=0;
static ssize_t db_value_store(struct class *class, 
			struct class_attribute *attr,	const char *buf, size_t count)
{	
	reg=simple_strtoull(buf,NULL,16);
	reg=0xff & reg;	
	extcon_set_cable_state(mcharge_chip->edev, FSA9285_EXTCON_ACA, true);
	return sprintf(buf,"Reg = %d \r\n",reg);
}
...
{
	u8 regval=0;
	regval = intel_mid_pmic_readb(reg);
	return sprintf(buf,"Reg[0x%x]=[%d] \r\n",reg,regval);	
}

static struct class_attribute emdoor_class_attrs[] = { 
	__ATTR(emdoor,0644,db_value_show,db_value_store),
    __ATTR_NULL
};

static struct class emdoor_interface_class = {
        .name = "emdoor_interface",
        .class_attrs = emdoor_class_attrs,
    };
...
static int __init fsa9285_extcon_init(void)
{
	...
	class_register(&emdoor_interface_class);
...
{
	class_unregister(&emdoor_interface_class);
