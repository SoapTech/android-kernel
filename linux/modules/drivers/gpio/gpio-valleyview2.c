...
static void vlv_update_irq_type(struct vlv_gpio *vg, unsigned type, void __iomem *reg, u32 offset)
{
	u32 value;
	...
	if (type & IRQ_TYPE_EDGE_BOTH) {
		...
		if (type & IRQ_TYPE_EDGE_FALLING)
			...
			value &= ~VV_TRIG_NEG;
		//pr_info("%s:%d. Checkpoint. reg = 0x%08x, value = 0x%08x, offset = %d.\n", __FUNCTION__, __LINE__, reg, value, offset);
	}
...
static int vlv_irq_type(struct irq_data *d, unsigned type)
{
	...
	*gpio_conf = type;
	...
	vlv_update_irq_type(vg, type, reg, offset);
...
static int vlv_gpio_get(struct gpio_chip *chip, unsigned offset)
{
	...
	retValue = readl(reg) & VV_LEVEL;
	retValue1 = readl(reg) & VV_LEVEL;
	pr_info("%s:%d. gpio_get value: %d, after read value %d, index offset: %d.\n", __FUNCTION__, __LINE__, retValue, retValue1, offset);
	#if 0 
...
static void vlv_gpio_dbg_show(struct seq_file *s, struct gpio_chip *chip)
{
	...
	for (i = 0; i < vg->chip.ngpio; i++) {
		...
		seq_printf(s, " gpio-%-3d %s %s %s pad-%-3d offset:0x%03x "
				"mux:%d %s %s %s. conf0 reg: 0x%08x, value := 0x%08x.\n",
			   i,
			   val & VV_INPUT_EN ? "  " : "in",
...
{
	struct vlv_gpio *vg = container_of(chip, struct vlv_gpio, chip);
	pr_info("%s:%d. 0x%08x, offset = 0x%08x.\n", __FUNCTION__, __LINE__, vg->irq_base, offset);
...
static void vlv_gpio_irq_dispatch(struct vlv_gpio *vg)
{
	...
	void __iomem *reg;
	u32 pending;
	if (vg->chip.ngpio == 102) {
		pr_info("%s:%d. Checkpoint gpio number = %d.\n", __FUNCTION__, __LINE__, vg->chip.ngpio);
	}
	for (base = 0; base < vg->chip.ngpio; base += 32) {
		reg = vlv_gpio_reg(&vg->chip, base, VV_INT_STAT_REG);
		pending = readl(reg);
		#if 0
		pr_info("%s:%d. Checkpoint pending GPIO interrupt. reg 0x08%x, pending 0x08x%.\n", __FUNCTION__, __LINE__, reg, pending);
		#endif
		while (pending) {
			pin = __ffs(pending);
			mask = BIT(pin);
			...
			#if 0
			if (vg->chip.ngpio == 102) {
				pr_info("%s:%d. Checkpoint. ngpio = %d, gpio_pin = %d.\n", __FUNCTION__, __LINE__, vg->chip.ngpio, pin);
			}
			#endif
			pending = readl(reg);
		}
