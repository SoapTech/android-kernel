...
static void fts_get_upgrade_info(struct Upgrade_Info *upgrade_info)
{
	...
	case IC_FT6208:
		...
		break;
	case IC_FT5926:
		upgrade_info->delay_55 = FT5926_UPGRADE_55_DELAY;
		upgrade_info->delay_aa = FT5926_UPGRADE_AA_DELAY;
		upgrade_info->upgrade_id_1 = FT5926_UPGRADE_ID_1;
		upgrade_info->upgrade_id_2 = FT5926_UPGRADE_ID_2;
		upgrade_info->delay_readid = FT5926_UPGRADE_READID_DELAY;
		upgrade_info->delay_earse_flash = FT5926_UPGRADE_EARSE_DELAY;
		break;
	default:
		break;
	}
...
int fts_ctpm_fw_read_app_5926(struct i2c_client *client, u8 *pbt_buf,
			  u32 dw_lenth)
{
	u32 packet_number;
	u32 j = 0;
	...
	pReadBuf = kmalloc(dw_lenth + 9, GFP_ATOMIC);
	packet_number = (dw_lenth) / FTS_PACKET_LENGTH;
	auc_i2c_write_buf[0] = 0x03;
	//auc_i2c_write_buf[0] = 0x85;
	auc_i2c_write_buf[1] = 0x00;
	...
	/*read app from flash and compart*/
	for (j=0; j<dw_lenth+8; j++) {
		if(pReadBuf[j] != pbt_buf[j]) {
			printk(KERN_WARNING " %X %X %X %d %d\n",pReadBuf[j],pbt_buf[j],j,lenght,dw_lenth);
			//kfree(pReadBuf);
			//return -EIO;
		}
	}
...
int fts_ctpm_fw_upgrade_5926_app(struct i2c_client *client, u8 *pbt_buf,
			  u32 dw_lenth)
{
	u8 reg_val[2] = {0};
	u32 i = 0;
	...
	u8 packet_buf[FTS_PACKET_LENGTH + 6];
	u8 auc_i2c_write_buf[10];
	u8 test_buf[FTS_PACKET_LENGTH];
	u8 bt_ecc;
	/* int i_ret; */
	struct Upgrade_Info upgradeinfo;

	fts_get_upgrade_info(&upgradeinfo);
	auc_i2c_write_buf[0] = 0x80;
	ft5x0x_i2c_read(client, auc_i2c_write_buf, 1,
		test_buf, 1);
	/* printk(KERN_WARNING "test_buf %02X \n",test_buf[0]); */
	...
	for (j = 0; j < packet_number; j++) {
		...
		ft5x0x_i2c_write(client, packet_buf, FTS_PACKET_LENGTH + 6);
		msleep(20);
		printk(KERN_WARNING "I2C Write %d \n",j);
	}
	...
	if (reg_val[0] != bt_ecc) {
		dev_err(&client->dev, "[FTS]--ecc error! FW=%02x bt_ecc=%02x\n",
					...
					bt_ecc);
		#if 0
		if(fts_ctpm_fw_read_app_5412(client, pbt_buf, dw_lenth) < 0) {
		dev_err(&client->dev, "[FTS]--app from flash is not equal to app.bin\n");
		//return -EIO;
		}
		#endif
	...
	auc_i2c_write_buf[0] = 0x07;
	ft5x0x_i2c_write(client, auc_i2c_write_buf, 1);
	msleep(5000);	/* make sure CTP startup normally */
...
void ft5926_InitResetPin(void)
{
	#if 1
	int ret = 0;
	//ret = gpio_request(FT5X0X_RESET_PIN, FT5X0X_RESET_PIN_NAME);
	ret = gpio_request_one(FT5X0X_RESET_PIN, GPIOF_OUT_INIT_LOW,
...
static int ft5926_CtrlResetPin(unsigned int value)
{
	gpio_set_value(FT5X0X_RESET_PIN, value);
	return 0;
}

void ft5926_FreeResetPin(void)
{
	ft5926_CtrlResetPin(1);
	msleep(1);
	gpio_free(FT5X0X_RESET_PIN);
}

void ft5926_Reset(void)
{
	gpio_set_value(FT5X0X_RESET_PIN, 0);
	msleep(1);
	gpio_set_value(FT5X0X_RESET_PIN, 1);
}
#endif
int fts_ctpm_fw_upgrade_5926_reset(struct i2c_client *client, u8 *pbt_buf,
			  u32 dw_lenth)
{
	u8 reg_val[2] = {0};
	u32 i = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		do {
			count++;
			i_ret = ft5x0x_i2c_write(client, auc_i2c_write_buf, 1);
			udelay(2000);
		} while (i_ret <= 0 && count < 5);
