...
//Add by emdoor jim.kuang 2014-04-28
static u8 ulpmc_debug_flag=0;
...
static struct ulpmc_chip_info *ulpmc_chip_ptr;
...
static int ulpmc_write_reg16(struct i2c_client *client, u8 reg, u16 value)
{
	...
	if (ret < 0)
		if (ulpmc_debug_flag != 0 ) dev_err(&client->dev, "I2C SMbus Write error:%d\n", ret);
...
static int ulpmc_read_reg16(struct i2c_client *client, u8 reg)
{
	...
	if (ret < 0)
		if (ulpmc_debug_flag != 0 ) dev_err(&client->dev, "I2C SMbus Read error:%d\n", ret);
...
static int ulpmc_throttle_chrg_cur(struct ulpmc_chip_info *chip, int lim)
{
	...
	case CHARGE_CURRENT_CNTL_LIM_MAX:
		...
		break;
	default:
		if (ulpmc_debug_flag != 0 ) dev_err(&chip->client->dev, "unknown limit:%d\n", lim);
		return -EINVAL;
	}
...
cc_throttle_fail:
	if (ret < 0)
		if (ulpmc_debug_flag != 0 ) dev_err(&chip->client->dev, "i2c write error:%d\n", ret);
	return ret;
}
...
static int byt_ulpmc_get_temp(int *temp)
{
	...
	if (ulpmc_debug_flag != 0 ) dev_info(&chip_ptr->client->dev,
...
static int ulpmc_charger_health(struct ulpmc_chip_info *chip)
{
	...
	if (stat < 0) {
		if (ulpmc_debug_flag != 0 ) dev_err(&chip->client->dev, "i2c read error:%d\n", stat);
	...
	if (not_chg_stat < 0) {
		if (ulpmc_debug_flag != 0 ) dev_err(&chip->client->dev,
				"i2c read error:%d\n", not_chg_stat);
		health = POWER_SUPPLY_HEALTH_UNSPEC_FAILURE;
		goto report_chrg_health;
	}
	...
	if ((fault & FAULT_VBUS_OVP) ||
		...
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "charger over voltage fault\n");
		health = POWER_SUPPLY_HEALTH_OVERVOLTAGE;
		goto report_chrg_health;
	}

	if ((fault & FAULT_CHRG_MASK) == FAULT_CHRG_THRM_FLT) {
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "charger over temp fault\n");
	...
	if (((fault & FAULT_CHRG_MASK) == FAULT_CHRG_TMR_FLT) ||
		...
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "charger timer fault\n");
		health = POWER_SUPPLY_HEALTH_UNSPEC_FAILURE;
		goto report_chrg_health;
	}

	if (!(stat & BC_STAT_PWR_GOOD)) {
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "charger low vbus fault\n");
...
static int ulpmc_battery_health(struct ulpmc_chip_info *chip)
{
	...
	if (fault & FAULT_BATT_FLT) {
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "battery over voltage fault\n");
		health = POWER_SUPPLY_HEALTH_OVERVOLTAGE;
		goto report_batt_health;
	}

	if (fault & FAULT_NTC_MASK) {
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "battery over temp fault\n");
		health = POWER_SUPPLY_HEALTH_OVERHEAT;
		goto report_batt_health;
	}
	...
	if ((batt_type & BATT_PRESENT_DET_MASK) != BATT_PRESENT_DET_2P) {
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "invalid battery!!\n");
		health = POWER_SUPPLY_HEALTH_UNKNOWN;
		goto report_batt_health;
	}

	if (stat & BC_STAT_VSYS_LOW) {
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "battery low fault\n");
		health = POWER_SUPPLY_HEALTH_DEAD;
		goto report_batt_health;
	}
...
static int ulpmc_battery_status(struct ulpmc_chip_info *chip)
{
	...
	default:
		ret = POWER_SUPPLY_STATUS_UNKNOWN;
		if (ulpmc_debug_flag != 0 ) dev_err(&chip->client->dev, "unkown battery status\n");
	}
...
{
	int ret = 0, batt_volt=0, cur_avg=0, temp=0;
...
static void dump_registers(struct ulpmc_chip_info *chip)
{
	...
	if (ret < 0)
		...
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "stat reg:%x\n", ret);

	ret = ulpmc_read_reg8(chip->client, ULPMC_BC_FAULT_REG);
	if (ret < 0)
		if (ulpmc_debug_flag != 0 ) dev_err(&chip->client->dev, "i2c read error:%d\n", ret);
	else
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "fault reg:%x\n", ret);

	ret = ulpmc_read_reg16(chip->client, ULPMC_FG_REG_FLAGS);
	if (ret < 0)
		if (ulpmc_debug_flag != 0 ) dev_err(&chip->client->dev, "i2c read error:%d\n", ret);
	else
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "fg flags reg:%x\n", ret);
}
...
static void log_interrupt_event(struct ulpmc_chip_info *chip, int intstat)
{
	...
	case INTSTAT_INSERT_AC:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "AC charger inserted\n");
		conn_evt = true;
		break;
	case INTSTAT_REMOVE_AC:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "AC charger removeed\n");
		break;
	case INTSTAT_INSERT_USB:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "USB charger inserted\n");
		conn_evt = true;
		break;
	case INTSTAT_REMOVE_USB:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "USB charger removed\n");
		break;
	case INTSTAT_START_CHARGE:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Charging start event\n");
		break;
	case INTSTAT_STOP_CHARGE:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Charging stop event\n");
		break;
	case INTSTAT_INSERT_BATTERY:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Battery inserted\n");
		break;
	case INTSTAT_REMOVE_BATTERY:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Battery removed\n");
		break;
	case INTSTAT_LOW_BATTERY:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "BATTLOW event!!\n");
		break;
	case INTSTAT_BTP_HIGH:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Battery Trip point high\n");
		break;
	case INTSTAT_BTP_LOW:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Battery Trip point low\n");
		break;
	case INTSTAT_THRM_BAT0:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Battery0 thermal event\n");
		break;
	case INTSTAT_THRM_BAT1:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Battery1 thermal event\n");
		break;
	case INTSTAT_THRM_SKIN0:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Skin0 thermal event\n");
		break;
	case INTSTAT_STOP_CHARGING:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Stop Charging/spurious event\n");
		break;
	case INTSTAT_PERC_HIGH:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Battery perc change high event\n");
		break;
	case INTSTAT_PERC_LOW:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "Battery perc change low event\n");
		break;
	case INTSTAT_FAULT_TRIGGER:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "fault event recieved\n");
		break;
	case INTSTAT_BATT_WARN:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev,
	...
	case INTSTAT_CHRG_OVP:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "charger over voltage evt\n");
		break;
	default:
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "spurious event!!!\n");
	}
...
static irqreturn_t ulpmc_thread_handler(int id, void *dev)
{
	...
	for (i = 0; i < ULPMC_INTR_QSIZE; i++) {
		...
		if (ret < 0) {
			if (ulpmc_debug_flag != 0 ) dev_err(&chip->client->dev, "ulpmc stat reg read error\n");
...
static void ulpmc_init_irq(struct ulpmc_chip_info *chip)
{
	...
	printk("########################## %s ######################\r\n",__func__);
	printk("########################## %s SB.GPO2_0x12=%d ######################\r\n",__func__,acpi_get_gpio("\\_SB.GPO2", chip->pdata->gpio));
	printk("########################## %s ######################\r\n",__func__);
	...
	if (ret) {
		...
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "IRQ No:%d\n", chip->client->irq);
	}
}
...
static void handle_extcon_events(struct ulpmc_chip_info *chip)
{
	...
	if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "extcon cur state:%d\n", event);

	if (!event) {
		chrg_type = POWER_SUPPLY_TYPE_USB_ACA;//POWER_SUPPLY_TYPE_USB;
		goto extcon_evt_ret;
	}
					...
					unsigned long event, void *data)
{
	...
	if (!chip->edev) {
		if (ulpmc_debug_flag != 0 ) dev_err(&chip->client->dev, "failed to get extcon device\n");
		return NOTIFY_OK;
	} else {
...
void ulpmc_fwupdate_enter(void)
{
	if (chip_ptr) {
		...
		if (ulpmc_debug_flag != 0 ) dev_info(&chip_ptr->client->dev, ":%s\n", __func__);
		mutex_lock(&chip_ptr->lock);
...
static void check_battery_presence(struct ulpmc_chip_info *chip)
{
	...
	ret = ulpmc_read_reg8(chip->client, ULPMC_BM_REG_BATT_PRESENT);
	if (ret < 0) {
		if (ulpmc_debug_flag != 0 ) dev_info(&chip->client->dev, "i2c read error:%d\n", ret);
		return ;
	}
				 ...
				 const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE)) {
		if (ulpmc_debug_flag != 0 ) dev_err(&client->dev,
	...
	if (ret < 0) {
		...
		if (ulpmc_debug_flag != 0 ) dev_err(&client->dev, "FG control reg:%x\n", ret);
	}
	chip = kzalloc(sizeof(*chip), GFP_KERNEL);
	if (!chip) {
		if (ulpmc_debug_flag != 0 ) dev_err(&client->dev, "failed to allocate memory\n");
		return -ENOMEM;
	}
	...
	if (ret) {
		if (ulpmc_debug_flag != 0 ) dev_err(&client->dev, "failed to register battery: %d\n", ret);
		goto probe_failed_1;
	}
	...
	if (ret) {
		if (ulpmc_debug_flag != 0 ) dev_err(&client->dev, "failed to register charger: %d\n", ret);
		goto probe_failed_2;
	}
	...
	chip->edev = extcon_get_extcon_dev(chip->pdata->extcon_devname);
	if (!chip->edev) {
		if (ulpmc_debug_flag != 0 ) dev_err(&client->dev, "failed to get extcon device\n");
	...
	if (ret < 0)
		if (ulpmc_debug_flag != 0 ) dev_err(&client->dev, "I2C write error:%d\n", ret);
...
{
	if (ulpmc_debug_flag != 0 )  dev_dbg(&client->dev, "ulpmc battery shutdown\n");
...
static int ulpmc_suspend(struct device *dev)
{
	...
	if (ulpmc_debug_flag != 0 )  dev_dbg(&chip->client->dev, "ulpmc battery suspend\n");
...
static int ulpmc_resume(struct device *dev)
{
	...
	set_s0ix_soc_thresholds(chip);
	if (ulpmc_debug_flag != 0 )  dev_dbg(&chip->client->dev, "ulpmc battery resume\n");
...
static int ulpmc_runtime_suspend(struct device *dev)
{
	...
	if (ulpmc_debug_flag != 0 )  dev_dbg(dev, "%s called\n", __func__);
	return 0;
}
...
static ssize_t ulpmcbat_value_store(struct class *class, 
			struct class_attribute *attr,	const char *buf, size_t count)
{	
	
	ulpmc_debug_flag = simple_strtoul(buf, NULL, 10);
	sprintf(buf,"ulpmc_debug_flag = %d \r\n",ulpmc_debug_flag);
	return count;
}

static ssize_t ulpmcbat_value_show(struct class *class, 
			struct class_attribute *attr,	char *buf)
{
	return sprintf(buf,"%s ulpmc_debug_flag = %d \r\n" , __func__,ulpmc_debug_flag);
}

static struct class_attribute ulpmcbat_class_attrs[] = { 
	__ATTR(ulpmcbat,0644,ulpmcbat_value_show,ulpmcbat_value_store),
    __ATTR_NULL
};

static struct class ulpmcdebug_interface_class = {
        .name = "ulpmcdebug_interface",
        .class_attrs = ulpmcbat_class_attrs,
    };
...
static int __init ulpmc_battery_init(void)
{
	...
	#if 1
	//register usr space
	ret = class_register(&ulpmcdebug_interface_class);
	if (ret) {
...
static void __exit ulpmc_battery_exit(void)
{
	...
	#if 1
	class_unregister(&ulpmcdebug_interface_class);
	#endif
}
