			...
			struct bq24192_chip *chip, bool val)
{
	int ret;
	u8 regval;
	dev_info(&chip->client->dev, "enable charging = %d\n", val);
	...
	ret = bq24192_write_reg(chip->client, BQ24192_INPUT_SRC_CNTL_REG, regval); if (ret < 0) {
		dev_err(&chip->client->dev,
...
#if 0
	/*WA to set SDP current to 500mA, fix me if root caused*/
	if (inlmt == 500) {
		regval1 = (regval & 0xF8)|0x06;
		bq24192_write_reg(chip->client, BQ24192_INPUT_SRC_CNTL_REG,
                                regval1);
		dev_warn(&chip->client->dev, "WA for inlmt  0x%d........\n", regval1);
	}
#endif
...
static int bq24192_charger_online(struct bq24192_chip *chip)
{
	u8 reg_val;

	reg_val = bq24192_read_reg(chip->client, BQ24192_SYSTEM_STAT_REG);

	if((reg_val & 0xC0) == 0x80)
		return 1;
	else
		return 0;
}
			...
			union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		...
		break;
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = chip->online | bq24192_charger_online(chip);
		break;
...
static void bq24192_handle_extcon_events(struct bq24192_chip *chip)
{
        int chrg_type, idx, ret;
        u32 event;
	dev_info(&chip->client->dev, "handle extcon event....\n");
...
extcon_evt_ret:
                chip->usb.type = chrg_type;
}

static int bq24192_extcon_cb(struct notifier_block *nb,
                                unsigned long old_state, void *data)
{
        struct bq24192_chip *chip = container_of(nb,
                                        struct bq24192_chip, ext_nb);

        bq24192_handle_extcon_events(chip);
	power_supply_changed(&chip->usb);
	printk("bq24192 extcon cb, handle power supply changed\n");
        return NOTIFY_OK;
}

static int bq24192_extcon_dev_reg_callback(struct notifier_block *nb,
                                        unsigned long event, void *data)
{
	...
	printk("bq24192 extcon dev reg callback\n");
        ...
        if (!chip->edev) {
                ...
                chip->ext_nb.notifier_call = &bq24192_extcon_cb;
			...
			const struct i2c_device_id *id)
{
	...
	chip->edev = extcon_get_extcon_dev("fsa9285");
        ...
        if (!chip->edev) {
                chip->ext_nb.notifier_call = &bq24192_extcon_dev_reg_callback;
                extcon_dev_register_notify(&chip->ext_nb);
        } else {
                chip->ext_nb.notifier_call = &bq24192_extcon_cb;
                ret = extcon_register_notifier(chip->edev, &chip->ext_nb);
                if (ret)
                        dev_err(&chip->client->dev, "failed to register extcon ntf");
        }

        if (chip->edev)
                bq24192_handle_extcon_events(chip);
#endif
	dev_info(&chip->client->dev, "bq24192 probe finished.......\n");
	return 0;
}
